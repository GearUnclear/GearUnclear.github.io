...
def load_csv_data(filepath):
    ...
    # Convert ID columns to integers, forcing NaNs to -1 (or another placeholder) before casting
...
def download_gif(gif_url, gif_path):
    try:
...
def show_gif(gif_path, window):
    if os.path.exists(gif_path):
        ...
        def update_frame(index):
            ...
            gif_window.after(50, update_frame, index)

        update_frame(0)

        def close_gif_window():
            gif_window.destroy()
            open_in_chrome("https://apricot.socialsolutions.com/report/run/report_id/138")

        gif_window.after(12000, close_gif_window)
    else:
        messagebox.showerror("Error", "GIF not available. Please try again later.")
...
def display_search_results(results, parent_frame):
    ...
    for i, client in enumerate(results):
        ...
        result_frame.grid_columnconfigure(0, weight=1)  # Make the label column expandable
        
        # Client info label with increased size and color
        client_info = tk.Label(
            result_frame, 
            text=f"{client['First']} {client['Last']} ({client['HMIS client number']}) - {client['Site']}: {client['xvxunit']}",
            font=('Arial', 12, 'bold'),
            fg='#0066cc'  # Dark blue color
        )
        ...
        # Assessment dropdown and button
        ...
        # Make all columns in the grid have equal width
        for col in range(8):
...
def fuzzy_search_execute(search_query, dataframe, results_frame):
    ...
    for index, row in dataframe.iterrows():
        ...
        name_score = fuzz.token_sort_ratio(search_query, f"{row['First'].lower()} {row['Last'].lower()}")
        unit_score = fuzz.ratio(search_query, str(row['xvxunit']).lower())
        max_score = max(hmis_score, name_score, unit_score)
        if max_score >= 40:  # Assuming 40 as the relevance threshold
    ...
    # Group matches by client and aggregate assessment data
    grouped_matches = {}
    for match in matches:
        key = (match[1], match[2], match[3], match[4], match[8], match[7], match[9], match[10])  # Include Site in the key
        if key not in grouped_matches:
            grouped_matches[key] = {
                ...
                'xvxunit': match[9],
                'Site': match[10],  # Include Site information
                'Assessments': []
            }
        grouped_matches[key]['Assessments'].append({'assessment_id': match[6], 'date': match[5]})

    # Sort the grouped matches by max_score in descending order
...
def open_profile_on_click(event, tree):
    item = tree.selection()[0]  # Get selected item
    profile_url = tree.item(item, 'values')[3]  # URL is in the fourth column
    open_in_chrome(profile_url)
...
#         result_frame.pack(padx=10, pady=5, fill='x')
#         
#         # Update the label to include Site information
#         tk.Label(result_frame, text=f"{client['First']} {client['Last']} - Site: {client['Site']} - Unit: {client['xvxunit']}").pack(side=tk.LEFT, padx=10)
#         
#         # Profile button
#         profile_button = tk.Button(
#             result_frame,
#             text="Profile",
#             command=lambda record_id=client['Record ID']: open_in_chrome(f"https://apricot.socialsolutions.com/document/edit/id/{record_id}")
#         )
#         profile_button.pack(side=tk.LEFT, padx=5)
#         # Family button
#         family_button = tk.Button(
#             result_frame,
#             text="Family",
#             command=lambda family_record_id=client['Family Record ID']: open_in_chrome(f"https://apricot.socialsolutions.com/document/edit/id/{family_record_id}")
#         )
#         family_button.pack(side=tk.LEFT, padx=5)
#         # Folder button
#         folder_button = tk.Button(
#             result_frame,
#             text="Folder",
#             command=lambda record_id=client['Record ID']: open_in_chrome(f"https://apricot.socialsolutions.com/profile/index/id/{record_id}")
#         )
#         folder_button.pack(side=tk.LEFT, padx=5)
#         assessment_var = tk.StringVar(result_frame)
#         assessment_button = tk.Button(result_frame, text="Assessment")
#         assessment_button.pack(side=tk.LEFT, padx=5)
# 
#         if client['Assessments']:
#             assessment_var.set(f"Assessment Date: {client['Assessments'][0]['date']}")
#             assessment_dropdown = tk.OptionMenu(
...
#                 command=lambda selected_date, a=client['Assessments'], b=assessment_button: update_assessment_link(selected_date, a, b)
#             )
#             assessment_dropdown.pack(side=tk.LEFT, padx=5)
#             update_assessment_link(assessment_var.get(), client['Assessments'], assessment_button)
#         
#         # Unit button
...
#             result_frame,
#             text="Unit",
#             command=lambda unit_record_id=client['Unit Record ID']: open_in_chrome(f"https://apricot.socialsolutions.com/document/edit/id/{unit_record_id}")
#         )
#         unit_button.pack(side=tk.LEFT, padx=5)
# 
#         # Update the assessment link initially
#         if client['Assessments']:
#             update_assessment_link(assessment_var.get(), client['Assessments'], assessment_button)
    ...
    results = [
        {
            ...
            'Site': row['Site'],  # Include Site information
...
def main():
    ...
    if selected_file:
        ...
        return
    # Process the CSV to format unit numbers and get the path of the processed file
